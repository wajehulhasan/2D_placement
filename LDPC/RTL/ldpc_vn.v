...
always @( * ) vnram_rdaddr_int <=  llr_access ? llr_addr : addr_vn;
...
always @( posedge rst, posedge clk )
  ...
  begin
    ...
    vnram_wraddr_int <=  llr_access ? llr_addr : vnram_rdaddr_aligned_msg;
    new_llr          <=  llr_access ? llr_din  : llrram_dout_aligned_msg;
    new_msg_sum      <=  llr_access ? 0        : msg_sum_reg;
    
    new_iteration    <=  llr_access | iteration;
    
    upmsg_we_int     <=  ~(llr_din_we | (we_vnmsg_aligned_msg & ~disable_aligned_msg));
  end
